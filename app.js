...
function generateSignatureHtml() {
    ...
    if (activeSocials.length > 0) {
        ...
        activeSocials.forEach(([net, url]) => {
            ...
            const iconUrl = hostedSocialUrls[net] || (window.location.origin + '/' + customSocialPaths[net]);
...
async function copySignatureToClipboard() {
    ...
    if (!hostedUrl || !hostedUrl.startsWith('http')) {
        alert("⚠️ Error: No se pudo hospedar la imagen del banner en internet.\n\nGmail tiene un límite estricto de 10,000 caracteres y no admite firmas con imágenes incrustadas largas. Verifica tu conexión a internet, asegúrate de que el servidor está corriendo y recarga la página antes de intentar de nuevo.");
        btn.disabled = false;
